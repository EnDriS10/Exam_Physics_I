...
for i= 1:length(t)-1
    ...
    %Inicialization
    r_inst= [x(i) y(i) z(i)];
    v_inst= [vx(i) vy(i) vz(i)];
    Fg_i = Fg(G, M, m, r_inst);
    Fr_i = Fr(R, h, mu, r_inst, v_inst);
    a = Get_a(Fg_i, Fr_i, m);
    
    ax(i) = a(1);
    ay(i) = a(2);    % Aceleraciones Instantaneas
    az(i) = a(3);

    % Predict position and velocity by Euler
    x_p = x(i) + vx(i) * dt;
    y_p = y(i) + vy(i) * dt;
    z_p = z(i) + vz(i) * dt;
    ...
    % Update position and velocity by RK
    
    x(i+1)=x(i)+0.5*(vx(i)+vx_p)*dt ;
    y(i+1)=y(i)+0.5*(vy(i)+vy_p)*dt ;
    z(i+1)=z(i)+0.5*(vz(i)+vz_p)*dt ;

    r_p= [x_p y_p z_p];
    v_p= [vx_p vy_p vz_p];
    Fg_p = Fg(G, M, m, r_p);
    Fr_p = Fr(R, h, mu, r_p, v_p);
    a_p = Get_a(Fg_p, Fr_p, m);   
    
    ax_p = a_p(1);
    ay_p = a_p(2);    %Aceleraciones predecidad por Euler
    az_p = a_p(3);

    vx(i+1)= vx(i)+ 0.5*(ax(i) + ax_p)*dt ;    
    vy(i+1)= vy(i)+ 0.5*(ay(i) + ay_p)*dt ; 
    vz(i+1)= vz(i)+ 0.5*(az(i) + az_p)*dt ; 
